...
Miscellaneous CalSim study variables span flow terms, return flows, allocations, and indices that do not fit the established categories. They use a range of reconstruction approaches for cases where standard quantile mapping is unsuitable or where a variable has its own governing logic.
...
Five different approaches are applied to reconstruct the miscellaneous terms:
...
SJR return flow at Woodbridge Irrigation District is agricultural drainage returning to the San Joaquin River system, cycling seasonally between near-zero winter values and peak return flows of approximately 0.7 TAF in active irrigation months. San Joaquin Valley Water Year Type monthly averaging assigns the mean monthly return flow of each of the five WYT classes to each synthetic month.
...
The reconstruction achieves $R^2 = 0.95$, $\text{NSE} = 0.95$, and $\text{PBIAS} = -1.4\%$. The monthly time series shows the reconstructed series closely tracking the seasonal oscillation of the actual record throughout the 1972-2018 period, with values cycling between near-zero winter lows and peaks of approximately 0.7 TAF in active irrigation months. The WYT conditioning captures the inter-annual differences in return flow magnitude, and the non-exceedance CDF shows close agreement across the full distribution, with only minor divergence at the very upper tail above the 95th percentile. The near-neutral percent bias of -1.4% confirms negligible long-term volume difference, and NSE = 0.95 indicates the WYT monthly averages capture the dominant seasonal and inter-annual structure of this regular irrigation-district return flow.

### R_RFS71A_OMR039_SV (Return Flow, San Joaquin River)

Westside SJR return flow at Byron Bethany Irrigation District is a second category of agricultural and municipal return flows to the San Joaquin River, with historical values reaching approximately 0.20 TAF. The same San Joaquin Valley Water Year Type monthly averaging is applied, identical in structure to the R_60N approach above.
...
East Bay Municipal Utility District terminal reservoir loss is the operational evaporation and seepage loss from EBMUD terminal storage facilities, with a consistent seasonal pattern between approximately 11 CFS in winter and 35 CFS in summer. Water year type averaging was selected for consistency with the broader project framework, though a repeating time series approach would also have been viable given the stable post-2009 behavior.
...
The Tule Wetlands Index tracks wetland conditions in the Tulare Basin, reconstructed through quantile mapping using VIC I_PEDRO (Don Pedro inflow) as the predictor, the highest R-squared match identified across the full VIC output screening. The QM relationship is trained on 1921-1971 and applied to 1972-2018. While the predictor correlation of $R^2 = 0.71$ sits at the lower threshold for effective quantile mapping, the approach preserves the statistical relationship between Don Pedro inflows and wetland conditions that WYT averaging alone cannot capture.
...
The moderate NSE of 0.52 reflects the difficulty QM faces in reproducing the timing and precise magnitude of individual peak events: the reconstruction over- and under-estimates specific peaks throughout the record, leading to the squared-deviation penalties that suppress NSE relative to $R^2$. The slight positive PBIAS of 5.0% indicates the reconstruction runs marginally high on average across the 1972-2018 period. Despite these limitations, $R^2 = 0.70$ (matching the predictor correlation) confirms the QM approach preserves the statistical relationship between Don Pedro inflows and wetland conditions that WYT averaging alone could not capture.
...
where $P_{Stockton}$ is monthly precipitation depth in inches from the Stockton gauge, $A_{Delta}$ is the Delta water surface area in acres varying across three defined time periods covering 1930-2010 land use changes, and $C_{ratio}$ is a watershed area adjustment coefficient.
