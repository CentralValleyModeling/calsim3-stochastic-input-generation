...
CalSimHydro is a water budget model that calculates agricultural and urban water demands, groundwater interactions, and return flows for 58 Water Budget Areas (WBAs) across California's Central Valley. It provides inputs for CalSim 3 including applied water demands, deep percolation, surface runoff, and actual evapotranspiration. By variable count, it is the largest single input category in the CalSim 3 stochastic inventory.
...
The version incompatibility first surfaced when the Rebalance model produced errors using L2020A CalSimHydro output, reporting that initial time series for areas "50" and "91" were missing. The Rebalance package ran successfully with the older L2015A output, but the L2020A output lacked these demand areas. Changing the PART F identifier from L2020A to L2015A in the DSS file did not resolve it. The root cause was missing demand units in the L2020A configuration: units 50_PA1, 91_PA, and 91_PR had to be added. An updated "Existing CSHydro" package with the corrected configuration defined the required demand areas and let the Rebalance module run.

## Results

Two model simulations were conducted to separate the effects of the new precipitation inputs from the evapotranspiration inputs, which allows the resulting differences to be attributed to each factor.

The ET-driven simulation showed that rangeland ET decreased under quantile-mapped inputs while agricultural ET increased slightly, reflecting the different response of managed and unmanaged land covers to ET adjustments. Lower rangeland ET leaves more water available at the soil surface for percolation, directly causing the +12% deep percolation increase seen across all WBAs. Applied water requirements for agriculture increased to meet higher potential ET, as expected when irrigation must compensate for elevated atmospheric demand.

The precipitation-driven simulation showed that lower WGEN precipitation (relative to CalSim 3 historical baseline inputs) translates directly to reduced surface runoff (-9%), with modest effects on other water budget components. CalSimHydro's surface runoff response is amplified relative to the precipitation change, consistent with rainfall-runoff theory: small changes in precipitation cause proportionally larger changes in runoff.
