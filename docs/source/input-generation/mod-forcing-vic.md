...
The `compile_rim_inflows` class (`_2_compile_rim_inflows.py`) routes VIC flux files into watershed-level monthly streamflow series that form the basis for downstream quantile mapping. Each watershed is defined by a grid-info file (`reference/GridInfo/*_GridInfo.txt`) listing its cells and per-cell area weights. For each grid cell in the watershed:
