...
For the historical validation reported below, the mapping is trained on WY 1922-1971 and validated on WY 1972-2018, so the corrected Product A flows are scored against historical CalSim 3 inputs from outside the training period. For rim inflow generation, the Product B mapping is trained on the full Product A overlap (Oct 1921-Dec 2018) and then applied to the 1,000-year stochastic VIC sequence.
