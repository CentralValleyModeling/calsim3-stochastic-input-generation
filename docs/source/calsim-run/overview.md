...
- **Regulatory compliance**: Biological opinion flow requirements, temperature management, minimum instream flows
- **System resilience**: Recovery time from drought sequences, minimum storage exceedance frequency, multi-year shortage characteristics
...
Model testing target: **February 2026**

Initial runs will use Product A (historical-length) inputs to verify that the stochastic framework reproduces baseline behavior before proceeding to the full Product B 1,000-year ensemble. This staged approach ensures any systematic biases identified during Product A validation can be diagnosed and addressed before committing to the computationally expensive Product B runs.
