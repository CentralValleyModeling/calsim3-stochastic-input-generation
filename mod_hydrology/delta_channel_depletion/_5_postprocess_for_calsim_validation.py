#%% Extract DCD (Delta Channel Depletion) Product A outputs for CalSim 1971-2018 validation.
#
# Reads the Product A DSS run and writes a standard validation CSV:
# Part B, Part C, Year, Month, Value.
#
# Usage:
#     python _2_postprocess_for_calsim_validation.py
#%%
...
DSS_SOURCES = [
    {
        "label":       "CS3sv_DCD_PRISM_Dtrnd",
        "inv_module":  "Delta Channel Depletion",
        "inv_filter":  "DCD_island_month.dss",
        "dss_path":    str(_dcd_runs / "DCD_Calsim3_PlanningStudy_ProductA_1921-2018" / "DCD" / "Output" / "CALSIM3" / "CS3sv_DCD_PRISM_Dtrnd.dss"),
        "dss_version": 7,
        "output_name": "_dcd_productA",
    },
]
...
def load_inventory_for_dss(inv_module, inv_filter):
    """Load and filter master inventory rows for a given module/DSS filename."""
    df_master = pd.read_excel(EXCEL_PATH, sheet_name=SHEET_NAME)
    rows = df_master[
        (df_master.iloc[:, 8] == inv_module) &
...
def extract_dss_data(dss_path, excel_partcs, dss_version=7):
    """Extract monthly time series from a DSS file, filtered to inventory parts."""
    data_dict = {}
    with HecDss.Open(str(dss_path), version=dss_version, catalog_flag=True) as dss:
...
def to_validation_csv(df, start_wy, end_wy):
    ...
    CFS_TAF_PER_DAY = 86400.0 / 43560.0 / 1000.0   # ≈ 0.001983471
    days = long.apply(
...
def main():
    ...
    print("=" * 80)
    print("Delta Channel Depletion — CalSim Historical Validation Extraction")
    ...
    for src in DSS_SOURCES:
        ...
        excel_partcs = load_inventory_for_dss(src["inv_module"], src["inv_filter"])
        print(f"  Inventory SVs: {len(excel_partcs)}")

        # Extract DSS data
        print(f"  Reading: {src['dss_path']}")
        df = extract_dss_data(src["dss_path"], excel_partcs, dss_version=src["dss_version"])
