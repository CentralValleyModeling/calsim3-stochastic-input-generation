...
# %% ── RESULTS ROOT ─────────────────────────────────────────────────────
BASE_RESULTS_DIR = _gen / "output"/"_3_run_hybrid_miscellaneous"
...
def prepare_hybrid_input_files(input_csv: Path) -> tuple[Path, Path]:
    """Build WYT and QMap runtime CSVs from a single hybrid_terms_miscellaneous.csv input."""
...
OUT_DIR = _gen / "output" / "_3_run_hybrid_miscellaneous"
FINAL_DIR = _gen / "output" / "_3_run_hybrid_miscellaneous"/ "hybrid_qmap_product_b_final"
...
HYBRID_WYT_FINAL_DIR = _gen / "output" / "_3_run_hybrid_miscellaneous" / "hybrid_WYT_product_b" / "hybrid_WYT_product_b_final"
HYBRID_QMAP_FINAL_DIR = _gen / "output" / "_3_run_hybrid_miscellaneous" / "hybrid_qmap_product_b_final"
HYBRID_FINAL_DIR = _gen / "output" / "_3_run_hybrid_miscellaneous" / "Hybrid_product_b_Final"


def read_final_hybrid_terms(input_csv: Path) -> pd.DataFrame:
    """Read target terms for the final hybrid step from hybrid_terms_miscellaneous.csv."""
    df = pd.read_csv(input_csv)
    df.columns = [str(c).strip().lower() for c in df.columns]
...
def main() -> None:
    ...
    cleanup_generated_outputs(
        base_dir=_gen / "output" / "_3_run_hybrid_miscellaneous",
